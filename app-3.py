...
def rank_technicians(service, area=None, limit=6):
    ...
    detail for the customer to decide before booking. Score weights, in
    order of importance:
      1. Same area as the customer (biggest boost — nearer = faster ETA)
      2. Paid 'featured' placement (still shown lower than a genuinely
         better-matched nearby technician — featured buys visibility, not
         a rigged #1 spot)
      3. Verified badge
    ...
    for r in rows:
        d = row2dict(r)
        score = 0.0
        if area and d.get("area") and area.strip().lower() == d["area"].strip().lower():
            score += 40
...
def technicians_suggest():
    ...
    full profile (name/shop, category, area, rating, jobs done, badges)."""
    service = request.args.get("service")
    if service not in DIAGNOSIS_RULES:
        return jsonify({"error": "service query param required"}), 400
    area = request.args.get("area")
    return jsonify(rank_technicians(service, area))
