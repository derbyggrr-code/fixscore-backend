...
import sqlite3, secrets, datetime, os, base64, json, re
import razorpay
...
# ------------------------------------------------------ Vision AI (photo) --
# Real photo analysis needs a Claude API key. Without it, the app still
# *requires* a photo + description from the customer (see /api/ai/estimate)
# but can't verify the photo actually shows the selected appliance — set
# ANTHROPIC_API_KEY as an environment variable (console.anthropic.com) to
# turn that on. Never hardcode the key here.
...
if ANTHROPIC_API_KEY:
    try:
        ...
        print(f"[vision-ai] anthropic client not available: {e}")

VISION_CATEGORIES = ["AC", "RO", "Fridge", "Washing Machine", "Electrician", "Plumber"]

def analyze_repair_photo(photo_base64, media_type="image/jpeg"):
    """Sends the customer's uploaded photo to Claude vision and asks it to
    (a) identify which of our 6 service categories the photo actually shows,
    and (b) describe the visible fault, in strict JSON. Returns None if no
    API key is configured (caller falls back to rule-based diagnosis without
    a category-match check) or if the model call/parse fails."""
    ...
    try:
        ...
        prompt = (
            "You are looking at a photo a customer uploaded of a broken home "
            "appliance/fixture for a repair-booking app. Categories we support: "
            "AC (air conditioner), RO (water purifier), Fridge, Washing Machine, "
            "Electrician (switches/wiring/MCB/panel), Plumber (pipes/taps/leaks). "
            "Look carefully at what is ACTUALLY in the photo.\n\n"
            "Respond with ONLY strict JSON, no other text, in exactly this shape:\n"
            '{"category": "<one of AC, RO, Fridge, Washing Machine, Electrician, '
            'Plumber, or Unclear>", "confidence": <0-100 integer>, '
            '"visible_issue": "<short factual description in Hinglish of what '
            'looks wrong, e.g. \\"AC outdoor unit ka coil dusty hai\\", or empty '
            'string if nothing visibly wrong>", "is_photo_of_appliance": <true/false>}'
        )
        resp = _anthropic_client.messages.create(
            ...
            messages=[{
                "role": "user",
                "content": [
                    {"type": "image", "source": {"type": "base64", "media_type": media_type, "data": photo_base64}},
        ...
        if data.get("category") not in VISION_CATEGORIES + ["Unclear"]:
            return None
        return data
    except Exception as e:
        print(f"[vision-ai] photo analysis failed: {e}")
        return None
...
def diagnose(service, symptoms, description="", has_photo=False, vision_result=None):
    """Turns free-text + a symptom checklist + an actually-analysed photo
    into a structured, explainable estimate.

    NOTE ON "AI" HERE: the symptom/keyword matching below is a rule-based
    engine (kept because it's fast and free). The photo, however, IS sent
    to a real vision model (see analyze_repair_photo()/ANTHROPIC_API_KEY
    above) when a key is configured — `vision_result` carries that model's
    actual reading of the image (visible_issue etc), which takes priority
    over the generic per-category text below."""
    ...
    if vision_result and vision_result.get("visible_issue"):
        issue = vision_result["visible_issue"]
        confidence += 22
        reasons.append("Photo AI-analysed — issue identified directly from what's visible in your photo")
    elif has_photo:
        confidence += 15
...
def ai_estimate():
    ...
    Assistant' card. Customer MUST provide a service category, a written
    description, AND a photo — the AI estimate is refused otherwise, and
    if a vision-AI key is configured (ANTHROPIC_API_KEY) the photo is
    actually analysed and checked against the selected category, so a
    customer can't get an AC diagnosis after uploading an RO photo."""
    x = request.json or {}
    ...
    photo_base64 = (x.get("photo_base64") or "").strip()
    has_photo = bool(photo_base64)

    if not description or len(description) < 3:
        return jsonify({"error": "Problem ko thoda describe karo (kam se kam ek line) — bina description ke AI sahi estimate nahi de sakta."}), 400
    if not has_photo:
        return jsonify({"error": "Photo upload karna zaroori hai — bina photo dekhe AI sahi diagnosis nahi de sakta."}), 400

    vision_result = None
    if _anthropic_client:
        vision_result = analyze_repair_photo(photo_base64)
        if vision_result is None:
            return jsonify({"error": "Photo analyse nahi ho payi — thodi behtar roshni/angle wali photo try karo."}), 400
        if vision_result.get("is_photo_of_appliance") is False:
            return jsonify({"error": "Ye photo kisi appliance/fixture ki nahi lag rahi. Kharab item ki saaf photo upload karo."}), 400
        detected = vision_result.get("category")
        if detected and detected != "Unclear" and detected != service and (vision_result.get("confidence") or 0) >= 55:
            return jsonify({
                "error": f"Photo mein {detected} dikh raha hai, lekin aapne '{service}' select kiya hai. "
                         f"Sahi service category select karo, ya {service} ki sahi photo upload karo.",
                "detected_category": detected,
                "category_mismatch": True,
            }), 400
    # NOTE: if no ANTHROPIC_API_KEY is configured, we can't verify the photo
    # actually matches the category — the app still requires a photo+description
    # (per the rules above) but falls back to rule-based diagnosis without the
    # cross-check. Set ANTHROPIC_API_KEY to turn on real photo verification.

    diag = diagnose(service, symptoms, description, has_photo, vision_result)
    return jsonify(diag)

@app.get("/api/public/reviews")
def public_reviews():
    """Public, no-login-required. Recent genuine customer reviews (4-5
    star, with a comment) for the homepage trust section — first name
    only, no contact info exposed."""
    rows = db().execute("""SELECT o.rating, o.feedback, o.service, c.name AS customer_name
                            FROM orders o LEFT JOIN users c ON c.id=o.customer_id
                            WHERE o.rating>=4 AND o.feedback IS NOT NULL AND TRIM(o.feedback)!=''
                            ORDER BY o.id DESC LIMIT 6""").fetchall()
    out = []
    for r in rows:
        first_name = (r["customer_name"] or "Customer").split(" ")[0]
        out.append({"rating": r["rating"], "comment": r["feedback"], "service": r["service"], "name": first_name})
    return jsonify(out)

@app.get("/api/public/stats")
def public_stats():
    """Public, no-login-required. A few safe aggregate numbers for the
    homepage trust strip (no revenue/commission/personal data)."""
    c = db()
    completed = c.execute("SELECT COUNT(*) n FROM orders WHERE status='completed'").fetchone()["n"]
    avg_rating = c.execute("SELECT AVG(rating) a FROM orders WHERE rating IS NOT NULL").fetchone()["a"]
    techs = c.execute("SELECT COUNT(*) n FROM users WHERE role='technician'").fetchone()["n"]
    return jsonify({
        "completed_orders": completed or 0,
        "avg_rating": round(avg_rating, 1) if avg_rating else None,
        "technician_count": techs or 0,
    })
