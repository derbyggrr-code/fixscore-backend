...
# ============================================================= PAYMENT ====
# SANDBOX ONLY — see module docstring. No money moves here.
...
def pay_order(order_id):
    ...
    if method not in ("upi", "card", "cash"):
        return jsonify({"error": "method must be upi, card or cash"}), 400
    c = db()
    o = c.execute("SELECT * FROM orders WHERE id=? AND customer_id=?", (order_id, u["id"])).fetchone()
    if not o:
        return jsonify({"error": "not found"}), 404
    ...
    status = "paid" if method != "cash" else "pay_on_visit"
    # TODO: for real UPI/card payments, create a Razorpay/PayU order here,
    # return their checkout params, and only set payment_status='paid'
    # inside the signed webhook callback — never straight from the client.

    # --- this is FixScore's cut: the whole reason the platform makes money ---
